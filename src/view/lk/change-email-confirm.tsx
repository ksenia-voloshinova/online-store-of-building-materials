...
import ChangeEmailConfirmForm from "@/shared/forms/changeEmailConfirmForm";
...
const ChangeEmailConfirm: NextPage = () => {
    const isAuth = useAppSelector(({ user }) => user.isAuth);

    return (
        <>
            {isAuth ? (
                <LkPageContainer title={"Подтверждение смены почты"}>
...
export default withAuthentication(withHeader(ChangeEmailConfirm));